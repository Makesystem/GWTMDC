...
public class RippleMixin<UIO extends MaterialUIObject & HasRipple> extends AbstractMixin<UIO> implements HasRipple {
	...
	@Override
	public void setRipple(final Color color) {
		this.color = color;
		
		uiObject.removeProperty(CssMixin.MDC_RIPPLE_SURFACE__COLOR);
		
		if (color != null)
			uiObject.setCssProperty(CssMixin.MDC_RIPPLE_SURFACE__COLOR, color);
	...
	private native void updateColor(final JavaScriptObject element, final String color)/*-{
        if (element) {
	        $jQuery(jsElement).find(".mdc-ripple-surface--color::before").css(
	                "background-color", color);
	        $jQuery(jsElement).find(".mdc-ripple-surface--color::after").css(
	                "background-color", color);
        }
	}-*/;
