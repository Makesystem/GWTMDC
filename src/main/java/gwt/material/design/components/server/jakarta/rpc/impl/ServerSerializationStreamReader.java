 ...
 * the License.
 */
package gwt.material.design.components.server.jakarta.rpc.impl;
...
import gwt.material.design.components.server.jakarta.Base64Utils;
import gwt.material.design.components.server.jakarta.rpc.RPC;
import gwt.material.design.components.server.jakarta.rpc.SerializationPolicy;
import gwt.material.design.components.server.jakarta.rpc.SerializationPolicyProvider;
...
public final class ServerSerializationStreamReader extends AbstractSerializationStreamReader {
	 ...
	 * (This is normally a hex string, but we allow a few more safe characters in
	 * case it's useful for testing.)
	 * 
	...
	/**
	 * Used to accumulate elements while deserializing array types. The generic type
	 * of the BoundedList will vary from the component type of the array it is
	 * intended to create when the array is of a primitive type.
	 * 
	...
	private enum VectorReader {
		...
		protected abstract Object readSingleValue(ServerSerializationStreamReader stream) throws SerializationException;
		...
		/**
		 * Convert a BoundedList to an array of the correct type. This implementation
		 * consumes the BoundedList.
		 */
		...
		Object read(ServerSerializationStreamReader stream, BoundedList<Object> instance, Type expectedType,
				DequeMap<TypeVariable<?>, Type> resolvedTypes) throws SerializationException {
			for (int i = 0, n = instance.getExpectedSize(); i < n; ++i) {
	...
	/**
	 * Used to look up setter methods of the form 'void Class.setXXX(T value)' given
	 * a Class type and a field name XXX corresponding to a field of type T.
	 */
	...
	public Object deserializeValue(Class<?> rpcType, Type methodType, DequeMap<TypeVariable<?>, Type> resolvedTypes)
			throws SerializationException {
		ValueReader valueReader = CLASS_TO_VALUE_READER.get(rpcType);
		if (valueReader != null) {
	...
	public void prepareToRead(String encodedTokens) throws SerializationException {
		...
		if (idx == 0) {
			...
			if (idx == 0) {
				...
				throw new IncompatibleRemoteServiceException(
						"Expecting version between " + SERIALIZATION_STREAM_MIN_VERSION + " and "
								+ SERIALIZATION_STREAM_MAX_VERSION + " from client, got " + version + ".");
			}
		}
		...
		if (getVersion() < SERIALIZATION_STREAM_MIN_VERSION || getVersion() > SERIALIZATION_STREAM_MAX_VERSION) {
			throw new IncompatibleRemoteServiceException("Expecting version between " + SERIALIZATION_STREAM_MIN_VERSION
					+ " and " + SERIALIZATION_STREAM_MAX_VERSION + " from client, got " + getVersion() + ".");
		}
	...
	public Object readObject(Type expectedType, DequeMap<TypeVariable<?>, Type> resolvedTypes)
		...
		if (isSimpleClass(expectedType) && !resolvedTypes.isEmpty()) {
			// Start a new scope for resolving type variables. This is a workaround for
			// false sharing
			// because the type checker doesn't implement type variables properly.
			//
			// For example, if expectedType is Serializable and the actual value is a
			// LinkedHashMap, we
			// don't want any of LinkedHashMap, HashMap, or Map to inherit any bindings for
			// their K,V
	...
	protected Object deserialize(String typeSignature, Type expectedType, DequeMap<TypeVariable<?>, Type> resolvedTypes)
			throws SerializationException {
		...
		try {
			...
			if (expectedType != null) {
				...
				expectedParameterTypes = SerializabilityUtil.findExpectedParameterTypes(instanceClass, expectedType,
						resolvedTypes);
			...
			instance = instantiate(customSerializer, instanceClass, expectedParameterTypes, resolvedTypes);
	...
	/**
	 * Deserialize an instance that is an array. Will default to deserializing as an
	 * Object vector if the instance is not a primitive vector.
	 * 
	...
	private void deserializeClass(Class<?> instanceClass, Object instance, Type expectedType,
		...
		/**
		 * A map from field names to corresponding setter methods. The reference will be
		 * null for classes that do not require special handling for server-only fields.
		 */
		Map<String, Method> setters = null;

		/**
		 * A list of fields of this class known to the client. If null, assume the class
		 * is not enhanced and don't attempt to deal with server-only fields.
		 */
		...
		for (Field declField : serializableFields) {
			...
			Method setter;
			/*
			 * If setters is non-null and there is a setter method for the given field, call
			 * the setter. Otherwise, set the field value directly. For persistence APIs
			 * such as JDO, the setter methods have been enhanced to manipulate additional
			 * object state, causing direct field writes to fail to update the object state
			 * properly.
			 */
		...
		if (serializationPolicy.shouldDeserializeFields(superClass)) {
			Type[] superParameterTypes = SerializabilityUtil.findExpectedParameterTypes(superClass, superClass,
					resolvedTypes);
			deserializeImpl(SerializabilityUtil.hasServerCustomFieldSerializer(superClass), superClass, instance,
					expectedType, superParameterTypes, resolvedTypes);
		}
	}

	private Object deserializeImpl(Class<?> customSerializer, Class<?> instanceClass, Object instance,
			Type expectedType, Type[] expectedParameterTypes, DequeMap<TypeVariable<?>, Type> resolvedTypes)
		...
		if (customSerializer != null) {
			...
			deserializeClass(instanceClass, instance, expectedType, expectedParameterTypes, resolvedTypes);
		}
	...
	private void deserializeStringTable() throws SerializationException {
		...
		for (int typeNameIndex = 0; typeNameIndex < typeNameCount; ++typeNameIndex) {
			...
			if (idx >= 0) {
				...
				while (idx >= 0) {
					...
					default:
						throw new SerializationException(
								"Unexpected escape character " + ch + " after backslash: \"" + str + "\"");
					}
					idx = str.indexOf('\\', pos);
				}
		...
		if (buffer.size() != buffer.getExpectedSize()) {
			throw new SerializationException(
					"Expected " + buffer.getExpectedSize() + " string table elements; received " + buffer.size());
		}
	...
	private void deserializeWithCustomFieldDeserializer(Class<?> customSerializer, Class<?> instanceClass,
			Object instance, Type[] expectedParameterTypes, DequeMap<TypeVariable<?>, Type> resolvedTypes)
	 ...
	 */
	private NumberFormatException getNumberFormatException(String value, String type, double minValue,
			double maxValue) {
	...
	private Object instantiate(Class<?> customSerializer, Class<?> instanceClass, Type[] expectedParameterTypes,
			DequeMap<TypeVariable<?>, Type> resolvedTypes) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SerializationException {
		if (customSerializer != null) {
			CustomFieldSerializer<?> customFieldSerializer = SerializabilityUtil
					.loadCustomFieldSerializer(customSerializer);
			if (customFieldSerializer == null) {
				Object result = instantiateWithCustomFieldInstantiator(customSerializer, expectedParameterTypes,
						resolvedTypes);
				if (result != null) {
					return result;
				}
	...
	private Object instantiateWithCustomFieldInstantiator(Class<?> customSerializer, Type[] expectedParameterTypes,
	...
	/**
	 * Returns true if the given type is a top-level class or interface with no type
	 * parameters.
	 */
	...
	private void validateTypeVersions(Class<?> instanceClass, SerializedInstanceReference serializedInstRef)
		...
		String serverTypeSignature = SerializabilityUtil.getSerializationSignature(instanceClass, serializationPolicy);
