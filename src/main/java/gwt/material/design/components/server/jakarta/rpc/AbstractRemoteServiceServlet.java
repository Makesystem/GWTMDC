...
public abstract class AbstractRemoteServiceServlet extends HttpServlet {
	...
	/**
	 * Override this method to control what should happen when an exception escapes
	 * the {@link #doPost} method. The default implementation will log the failure
	 * and send a generic failure response to the client.
	 * <p>
	 ...
	 * serialized back to the client, and are not passed to this method. This method
	 * is called only for exceptions or errors that are not part of the service
	 * method's signature, or that result from SecurityExceptions,
	 ...
	 * response AND to rethrow the exception, then this method should first send the
	 * GENERIC_FAILURE_MSG response itself (using getThreadLocalResponse), and then
	 * rethrow the exception. Rethrowing the exception will cause it to escape into
	...
	protected void doUnexpectedFailure(Throwable e) {
		try {
			...
			/*
			 * If we can't reset the request, the only way to signal that something has gone
			 * wrong is to throw an exception from here. It should be the case that we call
			 * the user's implementation code before emitting data into the response, so the
			 * only time that gets tripped is if the object serialization code blows up.
			 */
	 ...
	 * issued the request, or <code>null</code> if it could not be determined. This
	...
	/**
	 * Gets the <code>HttpServletResponse</code> object for the current call. It is
	 * stored thread-locally so that simultaneous invocations can have different
	 * response objects.
	 */
	 ...
	 * Because <code>doPost</code> swallows all <code>Throwables</code>, this method
	 * may throw any exception the implementor wishes.
	 */
	 ...
	 * For example, you may want to bypass the check of the Content-Type and
	 * character encoding headers in the request, as some proxies re-write the
	 ...
	 * HTTP/1.1 specification, which includes handling both requests which include a
	 * Content-Length header and requests utilizing <code>Transfer-Encoding:
	 * chuncked</code>.
	 * 
	 ...
	 * this class is constructed or deserialized. This method should be called from
	 * within a 'synchronized(this) {}' block in order to ensure that only one
	 * thread creates the objects.
	 */
