...
public class RPCServletUtils {
	...
	/**
	 * The UTF-8 Charset. Use this to avoid concurrency bottlenecks when converting
	 * between byte arrays and Strings. See
	 * http://code.google.com/p/google-web-toolkit/issues/detail?id=6398
	 */
	...
	/**
	 * Contains cached mappings from character set name to Charset. The null key
	 * maps to the default UTF-8 character set.
	 */
	private static final ConcurrentHashMap<String, Charset> CHARSET_CACHE = new ConcurrentHashMap<String, Charset>();
	 ...
	 * encoding. This is done by checking that the accept-encoding header specifies
	...
	/**
	 * Get the Charset for a named character set. Caches Charsets to work around a
	 * concurrency bottleneck in FastCharsetProvider. See
	 * http://code.google.com/p/google-web-toolkit/issues/detail?id=6398
	 * 
	 * @see {@link Charset#forName(String)}
	 *
	 * @param encoding the name of the Charset to get. If this is null the default
	 *                 UTF-8 character set will be returned.
	...
	/**
	 * Returns true if the {@link java.lang.reflect.Method Method} definition on the
	 * service is specified to throw the exception contained in the
	 ...
	 * validation is to be performed, and you are willing to allow for some types of
	 * cross type security attacks
	 * @param expectedCharSet     the expected request charset, or <code>null</code>
	 *                            if no charset validation is to be performed and
	 *                            <code>UTF-8</code> should be assumed
	 ...
	 *                          read from or closed
	 * @throws ServletException if the request's content type does not equal the
	 *                          supplied <code>expectedContentType</code> or
	...
	public static String readContent(HttpServletRequest request, String expectedContentType, String expectedCharSet)
		...
		/*
		 * Need to support 'Transfer-Encoding: chunked', so do not rely on presence of a
	 ...
	 *                          <code>gwt/x-gwt-rpc; charset=utf-8</code>, ignoring
	 *                          case
	 */
	...
	/**
	 * Returns the content of an {@link HttpServletRequest} by decoding it using the
	 * UTF-8 charset.
	 *
	 * @param request the servlet request whose content we want to read
	 * @return the content of an {@link HttpServletRequest} by decoding it using the
	 *         UTF-8 charset
	 ...
	 *                     that it matches the expected value 'text/x-gwt-rpc' and
	 *                     the content encoding is UTF-8. Disabling this check may
	 *                     allow some types of cross type security attacks.
	 * @return the content of an {@link HttpServletRequest} by decoding it using the
	 *         UTF-8 charset
	 * @throws IOException      if the requests input stream cannot be accessed,
	 *                          read from or closed
	 ...
	 * <code>gzipResponse</code> is <code>true</code>, the response content will be
	 * gzipped prior to being written into the response.
	 *
	 ...
	 * @param failure
	 */
	public static void writeResponseForUnexpectedFailure(ServletContext servletContext, HttpServletResponse response,
			Throwable failure) {
		...
		try {
			...
			servletContext.log("respondWithUnexpectedFailure failed while sending the previous failure to the client",
					ex);
		}
	}
	...
	private static void checkCharacterEncodingIgnoreCase(HttpServletRequest request, String expectedCharSet)
		...
		if (characterEncoding != null) {
			 ...
			 * instead of indexOf. Need to be sure that servlet engines return a properly
			 * parsed character encoding string if we decide to make this change.
			 */
	...
	/**
	 * Performs Content-Type validation of the incoming request, ignoring case and
	 * any <code>charset</code> parameter.
	 *
	...
	private static void checkContentTypeIgnoreCase(HttpServletRequest request, String expectedContentType)
		...
		if (contentType != null) {
			...
			/*
			 * NOTE:We use startsWith because some servlet engines, i.e. Tomcat, do not
			 * remove the charset component but others do.
			 */
