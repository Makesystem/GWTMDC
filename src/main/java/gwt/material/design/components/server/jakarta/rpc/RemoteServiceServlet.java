package gwt.material.design.components.server.jakarta.rpc;
...
public class RemoteServiceServlet extends AbstractRemoteServiceServlet implements SerializationPolicyProvider {
	 ...
	 * ServletContext as this servlet. Returns null if not found. (Used by
	...
	static SerializationPolicy loadSerializationPolicy(HttpServlet servlet, HttpServletRequest request,
		...
		/*
		 * Check that the module path must be in the same web app as the servlet itself.
		 * If you need to implement a scheme different than this, override this method.
		 */
	...
	/**
	 * The HTTP port of a Super Dev Mode code server running on localhost where this
	 * servlet will download serialization policies. (If set to zero, this feature
	 * is disabled and no download will be attempted.)
	 */
	private int codeServerPort = 0;

	/**
	 * The default constructor used by service implementations that extend this
	 * class. The servlet will delegate AJAX requests to the appropriate method in
	 * the subclass.
	 */
	...
	/**
	 * The wrapping constructor used by service implementations that are separate
	 * from this class. The servlet will delegate AJAX requests to the appropriate
	...
	/**
	 * Returns the value of the gwt.codeserver.port system property, or zero if not
	 * defined.
	 *
	 ...
	 *         {@value com.google.gwt.user.client.rpc.RpcRequestBuilder#MODULE_BASE_HEADER}
	 *         header
	 */
	 ...
	 * {@link RemoteServiceServlet#checkPermutationStrongName()} to prevent possible
	 * XSRF attacks and then decodes the <code>payload</code> using
	 * {@link RPC#decodeRequest(String, Class, SerializationPolicyProvider)} to do
	 * the actual work. Once the request is decoded
	 * {@link RemoteServiceServlet#processCall(RPCRequest)} will be called.
	 * <p>
	 * Subclasses may optionally override this method to handle the payload in any
	 * way they desire (by routing the request to a framework component, for
	 * instance). The {@link HttpServletRequest} and {@link HttpServletResponse} can
	 * be accessed via the {@link #getThreadLocalRequest()} and
	 ...
	 *                                that is not declared in the service method's
	 *                                signature
	 * @throws RuntimeException       if the service method throws an unchecked
	 *                                exception (the exception will be the one
	 ...
	 * component, for instance). The {@link HttpServletRequest} and
	 * {@link HttpServletResponse} can be accessed via the
	 * {@link #getThreadLocalRequest()} and {@link #getThreadLocalResponse()}
	 * methods.
	 * </p>
	 * This is public so that it can be unit tested easily without HTTP.
	 *
	 ...
	 * 
	 * This doPost method swallows ALL exceptions, logs them in the ServletContext,
	...
	/**
	 * Loads the {@link SerializationPolicy} for given module base URL and strong
	 * name. Returns the policy if successful or null if not found. Due to caching,
	 * this method will only be called once for each combination of moduleBaseURL
	 * and strongName.
	 * </p>
	 *
	 * <p>
	 * The default implementation loads serialization policies stored as servlet
	 * resources in the same ServletContext as this servlet.
	 *
	 * <p>
	 * Override this method to load the {@link SerializationPolicy} using an
	...
	/**
	 * Returns a URL for fetching a serialization policy from a Super Dev Mode code
	 * server.
	 *
	 * <p>
	 * By default, returns null. If the {@code gwt.codeserver.port} system property
	 * is set, returns a URL under {@code http://localhost:{port}}.
	 *
	 * <p>
	 * To use a server not on localhost, you must override this method. If you do
	 * so, consider the security implications: the policy server and network
	 * transport must be trusted or this could be used as a way to disable security
	 * checks for some GWT-RPC requests, allowing access to arbitrary Java classes.
	 *
	 * @param strongName the strong name from the GWT-RPC request (already
	 *                   validated).
	...
	/**
	 * Loads a serialization policy from a Super Dev Mode code server. (Not used
	 * unless {@link #getCodeServerPolicyUrl} returns a URL.)
	 *
	 * <p>
	 * The default version is a simple implementation built on java.net.URL that
	 * does no authentication. It should only be used during development.
	 * </p>
	 */
	...
	/**
	 * Override this method to examine the serialized response that will be returned
	 * to the client. The default implementation does nothing and need not be called
	 * by subclasses.
	 * 
	...
	/**
	 * Override this method to examine the serialized version of the request payload
	 * before it is deserialized into objects. The default implementation does
	 * nothing and need not be called by subclasses.
	 * 
