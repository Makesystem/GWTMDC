 ...
 *
 */
public class MaterialSelect<V> extends MaterialValuedField<V>
	...
	protected final ToggleStyleMixin<MaterialSelect<V>> unborderedMixin = new ToggleStyleMixin<>(this,
			CssName.MDC_SELECT__UNBORDERED);
	...
	protected native void addOnChangeEvent()/*-{
		var _this = this;
		var jsElement = this.@gwt.material.design.components.client.base.widget.MaterialWidget::jsElement;
		if (jsElement)
			jsElement.listen('MDCSelect:change', function(event) {
				_this.@gwt.material.design.components.client.ui.MaterialSelect::onSelect()();
			});
	}-*/;
	...
	public void setValue(final V value, boolean fireEvents) {
		if (initialized) {
			final Widget item = select.getChildrenList().stream()
					.filter(widget -> ((Option<V>) widget).getValue().equals(value)).findAny()
					.orElse(null);
			if (item == null)
				selectedIndex(-1);
			else
	...
	public void addOptions(@SuppressWarnings("unchecked") final V... values) {
		for (V value : values) {
			final MaterialSelect.Option<V> option = new MaterialSelect.Option<>();
