...
public class MdcDate implements HasPrevious<MdcDate>, HasNext<MdcDate>, Comparable<MdcDate> {
	...
	public MdcYear getMdcYear() {
		return month.getMdcYear();
	}
	...
	public MdcDate getTheClosest(final MdcDate... values) {
		...
		final long valueAsNumber = getTimestamp();

		return Arrays.stream(values).min(Comparator.comparing(value -> MathHelper.calcDifference(value.getTimestamp(), valueAsNumber)))
				.orElse(null);

	}
	
	public MdcDate getTheFarthestFrom(final MdcDate... values) {
		...
		return Arrays.stream(values).max(Comparator.comparing(value -> MathHelper.calcDifference(value.getTimestamp(), valueAsNumber)))
				.orElse(null);
